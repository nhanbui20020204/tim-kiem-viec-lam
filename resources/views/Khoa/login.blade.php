...
@section('noi_dung')
    <div id="app" class="d-flex col-lg-4 align-items-center auth-bg px-2 p-lg-5">
        <div class="col-12 col-sm-8 col-md-6 col-lg-12 px-xl-2 mx-auto">
            <h2 class="card-title fw-bold mb-1">Đăng Nhập Khoa</h2>
            <div class="mb-1">
            ...
            <p class="text-center mt-2"><span>Tạo tài khoản mới</span><a href="/khoa/register"><span>&nbsp;Tạo tài
                        khoản</span></a></p>
        </div>
    </div>
@endsection
        ...
        $(document).ready(function() {
            new Vue({
                ...
                methods: {
                    ac_login() {
                        axios
                            .post('{{ Route('actionLoginKhoa') }}', this.login)
                            .then((res) => {
                                if (res.data.status == 1) {
                                    toastr.success(res.data.message);
                                    setTimeout(() => {
                                        window.location.replace('/khoa/sinh-vien');
                                    }, 500);
